...
# 生成随机的 SMT 公式
def generate_random_smt_formula(num_formulas=100, max_depth=5, unsat_ratio=0.3):
    ...
    def generate_unsat_formula():
        # 创建不可满足公式，例如 x1 和 not x1
        var1 = random.choice(variables)
        return And(var1, Not(var1))  # 产生矛盾的公式

    formulas = []
    for _ in range(num_formulas):
        if random.random() < unsat_ratio:  # 控制 unsat 公式的比例
            formulas.append(generate_unsat_formula())
        else:
            formulas.append(generate_formula())
...
def smt_formula_to_smtlib(formula):
    if is_not(formula):
        ...
        return f"(=> {smt_formula_to_smtlib(formula.arg(0))} {smt_formula_to_smtlib(formula.arg(1))})"  # 使用 (=>)
...
# 保存随机生成的公式到文件
def save_formulas_to_files(formulas, output_dir="/home/clhiker/seeds"):
    # 创建输出目录（如果不存在）
    ...
    for i, formula in enumerate(formulas):
        ...
        filename = os.path.join(output_dir, f"{i}.smt2")
...
# 随机生成 100 个 SMT 公式
random_smt_formulas = generate_random_smt_formula(100, unsat_ratio=0.5)
